...
The Coulomb friction is modeled as an inclusion such as:
\begin{equation}\label{equation_A-S_columnbfriciton}
  \mathcal{F}(w,x) =
  \begin{cases}
    F_d(x), & \mbox{if } w>w_c \\
    [-F_c(x), F_c(x)], & \mbox{if } |w|<w_c \ \\
    -F_d(x), & \mbox{if } w < -w_c.
  \end{cases}
\end{equation}
where $w_c$ is the angular velocity threshold for Coulomb friction transition from static to dynamic. $F_d$ and $F_c$ are dynamic and static Coulomb torque, respectively. 

\subsection{Mathematical Background}\label{SubSec_AS_mathematicalbackground}
