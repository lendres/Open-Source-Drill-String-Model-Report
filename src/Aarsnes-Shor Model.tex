...
In summary, there are mainly two advantages of removing the PI controller in this project: \reviewcomment{Numbering implies an order or ranking.} 1) Obtain model with reduced parameters 2) Remove damping effect from the PI controller. \reviewcomment{If we are going to list things, let's use a list environment.  Otherwise, we can just write them in sentence form.} However, if the PI controller is available in the rig, simulation with PI controller can provide more accurate results for field application.
\begin{figure}[!hbt]
  \centering
