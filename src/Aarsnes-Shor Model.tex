...
The static friction is calculated based on the normal force along the drill string that is:
\begin{equation}\label{equation_A-S-Fn}
  F_N(x) = gA(x)(\rho-\rho_{mud})sin\theta
\end{equation}
where g is the gravitational force, A(x) is the cross sectional area at x, $\rho$ is the denstiy of drill string, $\rho_{mud}$ is the mud density, and $\theta$ is the inclination angle of deviated wel.
Then the static Coulomb friction is obtained as
\begin{equation}\label{equation_A-S_static_fric}
  F_c(x,\mu_s) = \mu_s r_o(x) F_N(x) 
\end{equation}
where $\mu_s$ is static frction factor, $r_o$ is outer radius of drill string and $F_N(x)$ is the normal force. The dynacmic friction is calulcated as a given ratio of static friction angle such as:
\begin{equation}\label{equation_A-S_dynamic_fric}
  F_d(x) = fRat\;F_c(x,\mu_s)
\end{equation}
where fRat is the pre-established ratio between dynamic and static friction ($0 \leq fRat \leq 1$). 

\section{Mathematical Background}
\subsection{Governing Equations}
The model is based on the equations of angular motion which are:
...
\begin{figure}[!hbt]
  \centering
  \includegraphics[width=6.5in]{Topdriveremove_MATLAB}
