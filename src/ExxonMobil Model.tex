...
The dynamic friction force opposes the direction of motion (\equationname~\ref{dynamic_force}). If, static friction applies, i.e., $\omega=0$; $V_{Sliding}=0$, then static friction force $F_{\mu,s}$ acts in the direction opposite to the sum of all frictional forces with a magnitude that equals the sum of all frictional forces. That is, the condition
...
If static friction force calculated above is less than $\mu_{s}\times F_{n}$, where $\mu_{s} > \mu_{k}$, then the surface is not sliding.
